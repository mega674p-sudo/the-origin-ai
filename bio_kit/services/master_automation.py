...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler("/home/ubuntu/the-origin-ai/automation_bio.log"),
        logging.StreamHandler()
    ]
)
logger = logging.getLogger("BioMasterAutomation")
...
HISTORY_FILE = "/home/ubuntu/the-origin-ai/bio_used_topics.json"
HASH_FILE = "/home/ubuntu/the-origin-ai/bio_uploaded_hashes.json"
...
def main():
    logger.info("Generating new biology short topic...")
    used = load_json(HISTORY_FILE)
    used_str = ", ".join(used[-10:]) if used else "None"
    
    topic_prompt = f"""
    Generate one fascinating, bizarre, mysterious, and engaging topic in Thai about BIOLOGY, nature, human body, strange animals, cells, genetics, or animal behaviors for a cinematic YouTube Short (Blurr Content style).
    ...
    try:
        ...
        logger.info(f"Selected Biology Topic: {topic}")
        used.append(topic)
    ...
    if not run_step("Dynamic Video Generation", ["python3", "/home/ubuntu/the-origin-ai/bio_kit/services/generate_dynamic_video.py", topic]):
        return
    ...
    # 3. Upload to YouTube
    yt_title = f"{topic} #Shorts #Biology #Nature #BlurrContent"
    yt_desc = f"สำรวจความลับแห่งโลกชีวะและธรรมชาติ: {topic} ในสไตล์สารคดีชวนตะลึง"
    ...
    logger.info("Master biology automation completed successfully!")
